...
export class AcompanharPedidoComponent implements OnInit, OnDestroy {
  ...
  steps = [
    { status: 1, label: 'Enviado', icon: 'bi-send' },
    ...
    { status: 5, label: 'Entregue', icon: 'bi-house-door-fill' }
  ];
  ...
  normalizarStatus(status: any): number {
    ...
    const mapa: any = { 'criado': 1, 'confirmado': 2, 'empreparo': 3, 'saiuparaentrega': 4, 'entregue': 5, 'finalizado': 5, 'cancelado': 0 };
    return mapa[status?.toString().toLowerCase()] || 1;
  }
